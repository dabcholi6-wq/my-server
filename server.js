...
const VideoSchema = new mongoose.Schema({
  ...
  title: String, order: Number, filename: String, filePath: String,
  createdAt: { type: Date, default: Date.now }
});
...
app.delete("/api/videos/:id", authMiddleware, async (req, res) => {
  ...
  if (video?.filePath) {
    const p = path.join(__dirname, video.filePath);
...
app.post("/api/activate-code", async (req, res) => {
  ...
  res.json({ message: "✅ کد با موفقیت فعال شد!", bundleId: accessCode.bundleId });
});
